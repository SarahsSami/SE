    ...
    try
{
    ...
    String sql = "SELECT  users.Name, users.email, explore.emp_id_x , explore.kw_explore ,explore.exp_explore ,explore.ss_explore ,explore.explore_status FROM (( explore INNER JOIN users ON users.ID = explore.emp_id_x)  INNER JOIN teamleader ON teamleader.emp_id = users.ID )WHERE teamleader.tl_id="+tid;
//String sql = "SELECT  users.Name, users.email, explore.emp_id_x, explore.kw_explore ,explore.exp_explore ,explore.ss_explore ,explore.explore_status  "
       // + "FROM users, explore "
        //+ "WHERE users.ID = explore.emp_id_x ";
...
while(rs.next())
{
ac =rs.getInt("explore_status");
 if(ac==0){
 %>
                
                <tr>
                    <form action = "explore_TL.jsp" method = "post">
                    <th> <%=rs.getString("Name")%></th>
                    <th> <%=rs.getInt("emp_id_x")%></th>
                    <input type="hidden" name="n" value="<%=rs.getString("Name")%>">
                    <input type="hidden" name="i" value="<%=rs.getInt("emp_id_x")%>">
                    <input type="hidden" name="kw" value="<%= rs.getString("kw_explore")%>">
                    <input type="hidden" name="exp" value="<%= rs.getString("exp_explore")%>">
                    <input type="hidden" name="ss" value="<%= rs.getString("ss_explore")%>">
                    <input type="hidden" name="e" value="<%= rs.getString("email")%>">
                   <th><input type="submit" value="EXPLORE" /></th> </form>
                </tr>
