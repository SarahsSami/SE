    ...
    try
{
    ...
    String sql = "SELECT  users.Name, users.email, assessments.emp_id_s , assessments.kw_assess ,assessments.exp_assess ,assessments.ss_assess ,assessments.assess_status FROM (( assessments INNER JOIN users ON users.ID = assessments.emp_id_s)  INNER JOIN teamleader ON teamleader.emp_id = users.ID )WHERE teamleader.tl_id="+tid;
//String sql = "SELECT  users.Name, users.email, assessments.emp_id_s , assessments.kw_assess ,assessments.exp_assess ,assessments.ss_assess ,assessments.assess_status  "
        //+ "FROM users, assessments "
       // + "WHERE users.ID = assessments.emp_id_s ";
...
ac =rs.getInt("assess_status");
 ...
 %>
                ...
                <tr>
                    ...
                    <th> <%=rs.getInt("emp_id_s")%></th>
                    <input type="hidden" name="n" value="<%=rs.getString("Name")%>">
                    <input type="hidden" name="i" value="<%=rs.getInt("emp_id_s")%>">
                    <input type="hidden" name="kw" value="<%= rs.getString("kw_assess")%>">
                    <input type="hidden" name="exp" value="<%= rs.getString("exp_assess")%>">
                    <input type="hidden" name="ss" value="<%= rs.getString("ss_assess")%>">
                    <input type="hidden" name="e" value="<%= rs.getString("email")%>">
                   <th><input type="submit" value="ASSESS" /></th> </form>
                </tr>
